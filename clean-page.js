  ...
  fetch(source, {cache:'no-store'}).then(r => {
    ...
    [...doc.scripts].forEach(old => {
      ...
      if (old.src) {
        ...
        if (/const menu=document\.getElementById\('menu'\)/.test(code)) return;
        s.textContent = code;
      }
